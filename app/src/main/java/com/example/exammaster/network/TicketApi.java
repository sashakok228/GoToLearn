...
public class TicketApi {
    ...
    public void createTicket(CreateTicketRequest request, String token, TicketCallback callback) {
        executor.execute(() -> {
            try {
                String response = postJson(
                        ApiConfig.BASE_URL + "/tickets",
                        buildTicketJson(request).toString(),
                        token
                );
    ...
    private JSONObject buildTicketJson(CreateTicketRequest request) throws Exception {
        JSONObject json = new JSONObject();

        json.put("subjectId", request.getSubjectId());
        json.put("ticketNumber", request.getTicketNumber());
        json.put("title", request.getTitle());
        json.put("content", request.getContent());

        JSONArray questionsArray = new JSONArray();

        for (CreateQuestionRequest question : request.getQuestions()) {
            JSONObject questionJson = new JSONObject();
            questionJson.put("questionNumber", question.getQuestionNumber());
